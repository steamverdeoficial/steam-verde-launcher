...
const NAVIGATE_URL  = 'https://steamverde.net/novo';  
// URL para verificar status na API (opcional, mantendo estrutura) 
const API_CHECK_URL = 'https://steamverde.net/novo/wp-json/steamverde/v1/check-status'; 
...
const TITLE_BAR_CSS = ` 
  ...
  header, .site-header, #masthead, .elementor-location-header, .elementor-section-fixed, .elementor-sticky--active { 
      top: 32px !important; 
  } 
...
// MODIFICADO: Aceita appVersion agora
  ...
  if (!document.getElementById('sv-custom-titlebar')) { 
    ...
    bar.innerHTML = \` 
      ...
      <div class="sv-bar-controls"> 
        <span class="sv-version-tag">v${appVersion} [Aziris]</span> 
...
const CLICK_LISTENER_SCRIPT = ` 
  document.body.addEventListener('click', (e) => { 
    ...
    if (link && link.href) { 
        if (link.href.startsWith('magnet:') || link.href.endsWith('.torrent') || link.href.includes('#') || link.href.startsWith('javascript:') || link.href === window.location.href) return; 
...
// --- NOVA FUNÇÃO: BARRA DE PROGRESSO NA SPLASH SCREEN --- 
function updateSplashStatus(text, percent = null) { 
    if (loadingWindow && !loadingWindow.isDestroyed()) { 
        // Código para injetar e atualizar a barra de progresso e texto 
        const code = ` 
            const textEl = document.querySelector('.loading-text'); 
            if(textEl) textEl.innerText = "${text}"; 
             
            // Se tiver porcentagem, cria/atualiza a barra 
            if (${percent !== null}) { 
...
function createSiteWindow(targetUrl) { 
  ...
  siteWindow.webContents.on('dom-ready', async () => { 
    try { 
        ...
        // MODIFICADO: Passa app.getVersion() aqui
...
// --- EVENTOS DO UPDATER COM BARRA DE PROGRESSO --- 
...
// Aqui mandamos a porcentagem para a Splash Screen 
autoUpdater.on('download-progress', (p) => {  
    if (loadingWindow && !loadingWindow.isDestroyed()) { 
        const percent = Math.round(p.percent); 
        updateSplashStatus('Baixando: ' + percent + '%', percent);  
    } 
}); 

autoUpdater.on('update-downloaded', () => {  
    if (loadingWindow && !loadingWindow.isDestroyed()) { 
        updateSplashStatus('Instalando...', 100);  
...
async function checkLoginAndStart() { 
    ...
    try { 
        ...
        if (authCookie && authCookie.expirationDate > now) { 
            // Tenta pegar o username do cookie 
            const cookieVal = decodeURIComponent(authCookie.value); 
            const username = cookieVal.split('|')[0]; 
             
            currentUser.name = username || "Assinante";  
...
ipcMain.on('login-success', async (event, data) => { 
  if (!data || !data.cookieValue || !data.cookieName) { console.error("Tentativa de acesso sem cookie."); return; } 
   
  // Força 1 ano de validade no cookie para garantir o auto-login 
  ...
  // Usa o nome que veio do login (data.user_display_name do renderer) 
  // Se não vier, tenta pegar do cookie 
  let username = data.user_display_name; 
  if (!username) username = data.cookieValue.split('|')[0]; 
