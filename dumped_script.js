  ...
  function svFindGameImage() {
    ...
    window.ipc.onOpenProfileData((event, data) => {
        ...
        listGames.innerHTML = '';
        // listGames.style.display lines removed to let Tab Switcher control visibility
        if(data.library && data.library.length > 0) {
             // Only set grid template here if we want to ensure it persists when visible
    ...
    window.svHideTooltip = function() {
        ...
        if(window.svUserInfo && window.svUserInfo.id && data.id) {
             ...
             // Mostrar apenas se IDs forem diferentes E meu ID não for 0/Guest
    ...
    window.ipc.onFriendRequestsUpdate((event, requests) => {
        ...
        if(requests && requests.length > 0) {
            ...
            container.style.cssText = 'background:#2a2e38; margin-bottom:10px; border-bottom:1px solid #a4d007;';
            let html = '<div style="background:#a4d007; color:#1b2838; font-weight:bold; padding:5px 10px; font-size:12px;">SOLICITAÇÕES PENDENTES</div>';
            requests.forEach(req => {
                html += '<div style="display:flex;align-items:center;padding:10px;gap:10px;border-bottom:1px solid #333;"><img src="' + (req.avatar || 'https://secure.gravatar.com/avatar/?d=mm') + '" style="width:30px;height:30px;border-radius:50%"><div style="flex:1"><div style="color:#fff;font-weight:bold;font-size:13px;">' + req.name + '</div><div style="color:#8f98a0;font-size:10px;">Quer ser seu amigo</div></div><button onclick="window.ipc.acceptFriend(' + req.id + ')" style="background:#a4d007;border:none;color:#1b2838;font-weight:bold;cursor:pointer;padding:4px 8px;border-radius:2px;">ACEITAR</button> <button onclick="window.ipc.rejectFriend(' + req.id + ')" style="background:#c21a1a;border:none;color:#fff;font-weight:bold;cursor:pointer;padding:4px 8px;border-radius:2px;">X</button></div>';
            });
    ...
    window.svSwitchProfileTab = function(tabName) {
        ...
        // Find triggering tab button (hacky but works if clicked)
        const tabs = document.querySelectorAll('.sv-profile-tab');
        ...
        if(content) {
            if(tabName === 'games') {
                ...
                content.style.gridAutoRows = 'max-content'; // Ensure rows respect content height
                content.style.alignItems = 'start'; // Prevent stretching overlap issues
